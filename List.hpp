...
public:
	...
	void splice(const_iterator position, List& x) {
		...
		while (first != last) {
			/* On garde la position dans x car first va �tre invalid� */
	...
	void merge(List &x, Compare comp) {
		if (&x != this) {
			/* On part du premier �l�ment de this */
			Node* actual = root->next;
			// Tant que x n'est pas vide
			while (x.elementCount > 0) {
				/* On prend toujours le premier �lement de x, qui va changer au fur et � mesure*/
				Node* firstX = x.root->next;

				/* Si on est � la fin, on ins�re avant la fin sans besoin de comparer */
				/* Si on est pas � la fin, on v�rifie si x est inf�rieur � la valeur actuelle, si oui on l'ins�re avant */
